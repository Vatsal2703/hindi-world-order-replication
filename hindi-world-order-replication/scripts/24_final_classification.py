...
# Collective models
base_cols = []
for col in ['info_status_diff', 'dep_len_diff', 'trigram_surprisal_diff',
            'lex_rept_surprisal_diff', 'lstm_surprisal_diff']:
    if col in available:
        base_cols.append(col)

if len(base_cols) >= 4:
    FEATURE_SETS['base1 = a+b+e+d+f'] = base_cols

if 'adaptive_surprisal_diff' in available and len(base_cols) >= 4:
    FEATURE_SETS['base1 + g'] = base_cols + ['adaptive_surprisal_diff']

# Without lex-rept
base2_cols = [c for c in base_cols if c != 'lex_rept_surprisal_diff']
if len(base2_cols) >= 3:
    FEATURE_SETS['base2 = a+b+e+f'] = base2_cols
...
# ============================================================================
full_model_name = 'base1 + g' if 'base1 + g' in FEATURE_SETS else list(FEATURE_SETS.keys())[-1]
