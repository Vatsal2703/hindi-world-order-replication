...
# Attach vanilla LSTM and adaptive LSTM scores to each variant
...
if len(trigram_filtered) == len(lstm_filtered):
    ...
    final = pd.merge(
        trigram_filtered,
        lstm_filtered[['sent_id', 'label', '_pair_rank'] + (['lstm_surprisal_diff'] +
            (['adaptive_surprisal_diff'] if 'adaptive_surprisal_diff' in lstm_filtered.columns else []))],
...
print(f"\n  Features present:")
print(f"    dep_len_diff:              {'✅' if 'dep_len_diff' in final.columns else '❌'}")
print(f"    info_status_diff:          {'✅' if 'info_status_diff' in final.columns else '❌'}")
print(f"    trigram_surprisal_diff:    {'✅' if 'trigram_surprisal_diff' in final.columns else '❌'}")
print(f"    lstm_surprisal_diff:       {'✅' if 'lstm_surprisal_diff' in final.columns else '❌'}")
print(f"    adaptive_surprisal_diff:   {'✅' if has_adaptive else '❌ (run script 16 first)'}")
print(f"    lex_rept_surprisal_diff:   {'✅' if has_lex_rept else '❌ (run script 21 first)'}")

print("\n" + "="*70)
print(" Next steps:")
missing_steps = []
if not has_adaptive:
    missing_steps.append("Run scripts/16_workflow_LSTM.py  (adaptive LSTM surprisal)")
if not has_lex_rept:
